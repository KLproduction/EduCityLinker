...
const AccommodationGalleryUpload = () => {
  const dispatch = useAppDispatch();
  const { onDeleteUploadcare, isPending: isDeleting } = useDeleteUploadcare();

  // Gallery in redux state is assumed to be an array of uploaded image URLs.
  const gallery: string[] =
    useAppSelector((state) => state.organization.amenityGallery) || [];
  ...
  const { uploadImageMutate, isPending } = useUploadRoomGallery();
  ...
  const handleRemoveUploaded = (id: string, index: number) => {
    ...
    dispatch(setOrganizationData({ amenityGallery: newGallery }));
  };
...
export default AccommodationGalleryUpload;
